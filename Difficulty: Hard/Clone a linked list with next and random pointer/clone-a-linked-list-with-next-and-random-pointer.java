...
class Solution {
    ...
    Node copyList(Node head) {
        ...
        Node curr=head;
        map.put(newhead.data,newhead);
        while(curr!=null){
            Node nex=curr.next;
            Node rand=curr.random;
            
            if(nex!=null){
                Node node = new Node(nex.data);
                newcurr.next=(map.containsKey(node.data))?map.get(node.data):node;
                if(!map.containsKey(node.data)) map.put(node.data,node);
            }
            if(rand!=null){
                Node node = new Node(rand.data);
                newcurr.random=(map.containsKey(node.data))?map.get(node.data):node;
                if(!map.containsKey(node.data)) map.put(node.data,node);
